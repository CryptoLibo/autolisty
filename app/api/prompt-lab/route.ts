...
The future output is an original, production-ready isolated graphic presented on one perfectly flat removable background color that does not appear anywhere inside the artwork. Prioritize the strength of the idea, visual variety, clear hierarchy, edge clarity and a composition that remains readable after the solid background is removed. Do not force a sticker treatment, white outline, unified backing shape or connected silhouette. Do not describe a shirt, mug, product, model, mockup or staged scene.
...
- Keep each final prompt concise: one paragraph of approximately 70-110 words. Use three compact parts in this order: core niche, subject and exact phrase; composition, style and palette; one short production clause covering only the removable solid background and clean isolation.
- Apply the detailed production checks internally, but do not copy this rule list, explain your reasoning, use headings, write negative-prompt inventories, or repeat the same constraint in different words inside the final prompt.
- Describe only the final isolated graphic: a balanced print-ready composition with generous clean margins, crisp edges, intentional contrast and freedom to use typography, illustration, separate supporting elements or other treatments when they strengthen the idea.
- Do not prescribe a sticker border, white outline, backing shape, connecting bridges or a single cut silhouette. Allow the image model to choose the most effective visual structure for each concept.
- Place the entire design on one perfectly uniform, flat, solid-color background. Choose a vivid background color that is clearly absent from every part of the artwork, including its text, illustration, accents and shadows. If a candidate color appears anywhere in the design, choose a different high-contrast background color.
...
- Keep the artwork fully opaque with clean separable edges against the background. Express this production requirement once through a concise positive description instead of enumerating every prohibited effect.
- Use intentional palette separation so the artwork remains readable independently of the removable background. Do not mention products or surfaces in the prompt.
...
- Include only SEO-useful details that materially identify the generated artwork: niche or identity, original phrase when present, main subjects, style, palette, typography, essential motifs, composition and mood. Omit decorative micro-details that do not change what the design is.
...
Before returning, silently reject and rewrite any concept that drifts from the primary niche, copies a reference, produces a mockup, unnecessarily forces a sticker treatment, uses a background color found inside the artwork, describes transparency or a checkerboard, adds variation to the solid background, or uses vague filler language.`,
          },
          ...referenceImages,
        ],
      },
